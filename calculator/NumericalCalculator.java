...
public class NumericalCalculator {
    // 操作数栈
    /*
     * private Stack<EmOperator> opStack = new Stack<EmOperator>(); //数据栈 private Stack<BigDecimal> dataStack = new
     * Stack<BigDecimal>();
     */
    // 负号替代符
    ...
    /** 计算主逻辑 */
    private static double _cal(String expression, Stack<EmOperator> opStack, Stack<BigDecimal> dataStack)
        ...
        for (int i = 0; i < length;) {
            ...
            if (Character.isDigit(value) || NEGATIVE_SIGN == value) {
                ...
                if (!EmOperator.RIGHT_BRACKET.equals(curOp = EmOperator.getByExpression(String.valueOf(value)))
                        && (opStack.isEmpty() || EmOperator.LEFT_BRACKET.equals(stackOp = opStack.peek())
                                || stackOp.getPriority() < curOp.getPriority())) {
                    opStack.push(curOp);
                    ++i;
                } else {
                ...
                if (charArr[i] != '(')
                    throw new ExpressionFormatException("公式[" + curOp + "]格式不正确，缺少‘('");
                ...
                if (params.size() != curOp.getParamNumber())
                    throw new ExpressionFormatException("公式，参数个数不正确:[运算符号:" + curOp + "][需要参数个数:"
                            + curOp.getParamNumber() + "][入参:" + expression + "]");
                ...
                throw new ExpressionFormatException("未知字符:" + value);
            }
        }
    ...
    private static void singleCal(Stack<EmOperator> opStack, Stack<BigDecimal> dataStack)
            throws ExpressionFormatException {
        ...
        case MULTIPLY:
            ...
            break;
        case DIVIDE:
            result = firstValue.divide(secondValue, 15, RoundingMode.HALF_UP).doubleValue();
            break;
        case MOD:
        ...
        case SQRT:
            if (firstValue.doubleValue() < 0)
                throw new ExpressionFormatException("SQRT公式参数小于零:" + firstValue);
    ...
    private static String preProcess(String expression, double... values) throws ExpressionFormatException {
        ...
        if (arr[0] == '*' || arr[0] == '/' || arr[0] == '%' || arr[arr.length - 1] == '*' || arr[arr.length - 1] == '/'
                || arr[arr.length - 1] == '%')
    ...
    /** 通过数组的方式，计算 */
    public static List<BigDecimal> cal4Arr(String expression, List<BigDecimal>... params)
            throws ExpressionFormatException {
        if (params.length < 1)
            throw new ExpressionFormatException("参数数目不正确[" + expression + "][params=" + params + "]");
        int paramLen = params[0].size();
        for (List<BigDecimal> valueList : params) {
            if (paramLen != valueList.size())
                throw new ExpressionFormatException("参数列表长度不相等[" + expression + "][params=" + params + "]");
        }
     ...
     * @throws ExpressionFormatException
     */
    public static BigDecimal calArrList(String expression, List<BigDecimal> params) throws ExpressionFormatException {
        if (params.size() < 1)
            throw new ExpressionFormatException("参数数目不正确[" + expression + "][params=" + params + "]");
        expression = preProcess(expression);
        int paramsCount = 0;
        ...
        if (paramsCount != params.size())
            throw new ExpressionFormatException("参数数目不正确[" + expression + "][params=" + params + "]");
        double[] singleParams = new double[params.size()];
        for (int i = 0; i < params.size(); ++i) {
            singleParams[i] = params.get(i).doubleValue();
        }
        return new BigDecimal("" + cal(expression, singleParams)).setScale(4, RoundingMode.HALF_UP);
    }
}