...
!(function($, window, doucument) {
    ...
    var Const = {
        ...
        /* 公开的函数 */
        EXPOSE_METHODS : ['setValue', 'getValue', 'setDisabled', 'isDisabled', 'getText', 'loadAjaxData']
    };
    ...
    // 重设下拉菜单的位置
    ComboZTree.prototype.place = function() {
        // 如果不是显示在Body区域，则不需要调正
    ...
    ComboZTree.prototype.initUI = function() {
        ...
        // 如果需要显示在body中
        if (this.options.appendToBody) {
            var menuId = this.$dom.attr("id") + "__DropMenu";
            var menuWrapper = $('<div class="rythm-comboztree-menu-wrapper">');
            this.$btnDrop.attr("href", "#" + menuId)
            this.$dropMenu.attr("id", menuId);
            this.$dropMenu.addClass("append-to-body");
            menuWrapper.appendTo("body");
            this.$dropMenu.appendTo(menuWrapper);
        }
    }
    ...
    ComboZTree.prototype.initEvent = function() {
        var that = this;
        // 下拉框的点击事件
        this.$dropMenu.click(function(event) {
            // 如果是阻止下拉框关闭
        ...
        this.$dropMenu.parent().on('show.bs.dropdown', function() {
            // 清空过滤器内容
            that.clearFilter();
            // 计算宽度
            that.resetMenuWidth();
    ...
    ComboZTree.prototype.loadAjaxData = function(ajaxResult) {
        ...
        var setting = {
            check : {
                chkboxType : {
                    "Y" : that.options.yChkboxType,
                    "N" : that.options.nChkboxType
                },
                enable : true,
                /* Check的类型 */
        ...
        } else if (this.options.expandLevel > 0) {
            var expandLevel = this.options.expandLevel;
            // 获取需要展开的节点
            var expandNodes = this.ztree.getNodesByFilter(function(node) {
                return node.level < expandLevel;
            }, false);
            // 展开根节点
            for (var i in expandNodes) {
                this.ztree.expandNode(expandNodes[i], true, false, true);
            }
        }
    ...
    $.fn.ComboZTree.defaults = {
        ...
        /* 父子联动（取消勾选） */
        nChkboxType : "ps",
        /* 菜单的默认容器(是否显示于Body中)-Since_0.0.17 */
        appendToBody : false,
        /* 数据编辑 */
