...
var Dialog = App.Dialog = function($, window, doucument) {
    ...
    function fnGetIframeTmp(dialogId, iframeUrl, iframeHeight, showLoadding) {
        ...
        htmlTmp.push('     style="width:0;height:0;border:none">');
        htmlTmp.push('</iframe>');
        htmlTmp.push('</div>');
        return htmlTmp.join('');
    }
            ...
            onshown : function(dialog) {
                ...
                if (dialog.options.bShowLoading) {
                    ...
                    iframe.load(function() {
                        ...
                        //iframe.show();
                        iframe.css({width:"100%", height: dialog.options.iframeHeight + "px"});
                    });
                } else {
                    //iframe.show();
                	iframe.css({width:"100%", height: dialog.options.iframeHeight + "px"});
...
//判断数组与字符串, 并统一处理为字符串返回
function isArrayFntoStr(value) {
    var key = false;
    if (typeof Array.isArray === "function") {
        key = Array.isArray(value);
    } else {
        key = Object.prototype.toString.call(value) === "[object Array]";
    }
    if(key){
        return value.join(",");
    } else {
...
// 处理所有的数据字段
function checkisArrayFntoStr(sConidtion){
    for (x in sConidtion) {  
        sConidtion[x] = isArrayFntoStr(sConidtion[x]);
    }  
    return sConidtion;
}

//判断时间是否符合标准
function isCorrectTime(id1,startDate,endDate,id2){
	var dateString = $("#"+id1).find(".datepickerinput").val();
...
//确定区间值
function getInputValue(data,standards,id){
	var datas = JSON.parse(standards);
	for(var i=0;i<datas.length;i++){
		if(datas[i].constant==""||datas[i].constant==null){
			if(datas[i].min == ""||datas[i].min==null){
				if(data<datas[i].max){
					$('#'+id).val(datas[i].name);
				}
			}else if(datas[i].max == ""||datas[i].max==null){
				if(data>=datas[i].min){
					$('#'+id).val(datas[i].name);
				}
			}else{
				if(data>=datas[i].min&&data<datas[i].max){
					$('#'+id).val(datas[i].name);
				}
			}
		}else{
			if(datas[i].constant.indexOf(data)){
				$('#'+id).val(datas[i].name);
			}
		}
	}
...
// 查询数据库--初始化页面中的文件
function selectFileData(pid,groupid,fileitem,url,uploadUrl,initialPreviewDownloadUrl){
    Ajax.ajax({
        url : url,
        data : {
            pid : pid,
        ...
        success : function(data) {
            initFileInput(pid,groupid,fileitem,uploadUrl,initialPreviewDownloadUrl,data.data);
        }
    }); 
}

// 文件控件初始化
function initFileInput(pid,groupid,ctrlName,uploadUrl,initialPreviewDownloadUrl,data) {
...
           return data; 
        },
        allowedFileExtensions: ['jpg', 'gif', 'png','bmp','doc','docx','pdf','ppt','pptx','xls','xlsx','txt'],//接收的文件后缀
    ...
    }).on('filebatchuploadsuccess', function(event) {  
        console.log('filebatchuploadsuccess！');
        allFileUploadSuccess();
...
// 所有文件上传成功后执行方法(默认方法)
function allFileUploadSuccess(){
    window.location.reload();
}

/**
 * 绑定 iCheck的全选和普通项目的连动
 * @param $iCheckAll 全选复选框（jQuery对象）
 * @param $iCheckOthers 其余复选框（jQuery对象）
 * @param fnChanged 回调函数
 * @param userData 回调函数参数（可选）
 */
function iCheckAllEventBind($iCheckAll, $iCheckOthers, fnChanged, userData) {
    // 全选
    $iCheckAll.off('ifChanged').on('ifChanged', function(event){
        if ($(this).data("onProgress")) { return; }
        // 自身调用
        $iCheckOthers.data("onProgress", true);
        // 其他按钮全选/全不选
        $iCheckOthers.icheck($(this).is(':checked')? 'checked' : 'unchecked');
        $iCheckOthers.data("onProgress", false);
        // 事件函数
        if ($.isFunction(fnChanged)) {
            fnChanged.apply(this, userData)
        }
    });
    // 其他选项
    $iCheckOthers.off('ifChanged').on('ifChanged', function(event){
        if ($(this).data("onProgress")) { return; }
        // 检查全选按钮
        var isAllChecked = true;
        $iCheckOthers.each(function(index) {
            if (!$(this).is(':checked')) {
                isAllChecked = false;
                return false;
            }
        });
        $iCheckAll.data("onProgress",true);
        $iCheckAll.icheck(isAllChecked ? 'checked' : 'unchecked');
        $iCheckAll.data("onProgress",false);
        // 事件函数
        if ($.isFunction(fnChanged)) {
            fnChanged.apply(this, userData)
        }
...
function downFile(key){
    window.location.href = 'uploadfile/downloadfile?key='+ key;
}
